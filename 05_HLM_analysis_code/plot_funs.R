plot_fun_1 <- function(ident,quant_method= "direct") {
  ...
  if(quant_method!="direct"){
    ...
    newdata$CIL<- exp(quants[1,])
    newdata$CIU <- exp(quants[2,])
  }
  ...
  plot1 <- ggplot(aes(x=Year),data=data)+
    ...
    geom_point(aes(y=y,colour= SiteName,shape= StudyName),alpha= 0.5,size= 1.5)+
    geom_line(aes(y=y,colour= SiteName),size=0.1,linetype=3)+
    geom_line(aes(y=pred,colour= SiteName,linetype= signif),alpha= 0.5,size=0.5)+
   
    geom_line(aes(y=pred,x= Year),data= newdata,
              linetype= p_val,colour= "black",size=0.8)+
  ...
  plot3 <- ggplot(aes(x=Year_group_cent),data=data)+
    geom_ribbon(aes(x=Year_group_cent,ymax= CIU_cent,ymin= CIL_cent),
                data= newdata,fill="grey50",alpha=0.5)+
    geom_line(aes(y=log_y_cent,colour= SiteName),size=0.1,linetype=3)+
    geom_point(aes(y=log_y_cent,colour= SiteName,shape= StudyName),alpha= 0.5,size=1.5)+
   
    geom_line(aes(y=log_pred_cent,colour= SiteName,linetype= signif),alpha= 0.5,size= 0.5)+
...
plot_fun_2 <- function(ident,quant_method= "direct") {
  ### gather group level data
  data <- subset(model_results,group_name==group_ID[ident])
  ...
  plot1 <- ggplot(aes(x=Year),data=data)+
    geom_ribbon(aes(x=Year,y= pred,ymax= CIU,ymin= CIL),
             data= newdata,fill="grey50",alpha=0.5)+
    geom_point(aes(y=y,fill= SiteName),alpha= 0.5,size= 0.5,shape= 21)+
    geom_line(aes(y=y,colour= SiteName),size=0.1,linetype=3)+
    geom_line(aes(y=pred,colour= SiteName,linetype= signif),size= 0.5,alpha= 0.5)+
    geom_line(aes(y=pred,x= Year),data= newdata, 
              linetype= p_val,colour= "black",size=1)+
    scale_y_continuous(breaks= c(0,0.5,1.0,1.5),labels= c(0,0.5,1.0,1.5))+
    ylab("Standardized Kelp Abundance+0.01")+
    options+
    xlab("Year")+
  ...
  plot3 <-  ggplot(aes(x=Year_group_cent),data=data)+
    geom_ribbon(aes(x=Year_group_cent,ymax= CIU_cent,ymin= CIL_cent),
                data= newdata,fill="grey50",alpha=0.5)+
    geom_point(aes(y=log_y_cent,fill= SiteName),alpha= 0.5,size= 0.5,shape= 21)+
    geom_line(aes(y=log_y_cent,colour= SiteName),size=0.1,linetype=3)+
    geom_line(aes(y=log_pred_cent,colour= SiteName,linetype= signif),alpha= 0.5,size=.5)+
    geom_line(aes(y=pred2,x= Year_group_cent),data= newdata,
              linetype= p_val,colour= "black",size=1)+
