plot_fun_1 <- function(ident) {
  ...
  minp <- ifelse(min(data$y)==0.01,0.009,min(data$log_y_cent)*0.87)
  ...
  plot1 <- ggplot(aes(x=Year),data=data)+
    ...
    geom_line(aes(y=pred,colour= SiteName),alpha= 0.5)+
    ...
    guides(colour= FALSE,shape= guide_legend(title="Study" ,title.hjust= 0.5,ncol= 2))+
  ...
  plot3 <- ggplot(aes(x=Year_group_cent),data=data)+
    ...
    geom_line(aes(y=log_pred_cent,colour= SiteName),alpha= 0.5)+
