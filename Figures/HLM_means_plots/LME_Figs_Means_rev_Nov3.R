...
setwd("/NCEAS/Time Paper/Time_Paper_Analyses")
...
ecoregiondata$posneg<-NA

if(ecoregiondata$sign > 0) {
  ecoregiondata$posneg<-"Pos"
} else if (ecoregiondata$sign < 0) {
  ecoregiondata$posneg<-"Neg"
} else {
  ecoregiondata$posneg<-"Neu"
}

ecoregiondata$posneg<- 
  if (ecoregiondata$sign > 0) {     
  print("pos") 
} else if (ecoregiondata$sign < 0) {     
  print("neg") 
...
addPosNeg <- function(ecoregiondata){
  
  #positive or negative based on achieving some fraction of a maximum possible slope
  ecoregiondata$posneg <- "Neu"
  ecoregiondata$posneg[which(ecoregiondata$upper_0.9_q < 0 & 
                         ecoregiondata$lower_0.9_q<0)] <- "Negative"
  ecoregiondata$posneg[which(ecoregiondata$upper_0.9_q > 0 & 
                               ecoregiondata$lower_0.9_q > 0)] <- "Positive"
ecoregiondata
}



ecoregiondata$DFO<-if (ecoregiondata$lower_0.9_q > 0) {     
 print("pos") 
} else if (ecoregiondata$lower_0.9_q < 0) {     
  print("neg") 
} else {     
  print("neu") 
} 
...
eco_mu_ALL$DF0 <- ifelse(sign(eco_mu_ALL$upper_0.9_q)==sign(eco_mu_ALL$lower_0.9_q),1,0)
eco_mu_ALL$DF0<- as.factor(eco_mu_ALL$DF0)
